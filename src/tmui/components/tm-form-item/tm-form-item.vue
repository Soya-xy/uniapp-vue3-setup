<template>
    <view :class="[`mx-${props.margin[0]}`]">
		<tm-sheet :transprent="props.transprent!==null?props.transprent:tmFormTransprent" :round="props.round" :margin="[0,0]" :padding="props.padding"  >
		    <view :class="[`py-${props.margin[1]}`]">
				...
				<view v-if="props.showError" style="height:30rpx;" >
					<tm-text v-if="tmFormFun=='validate'&&item.isRequiredError==true" color="red" :font-size="22" :label="item.message"></tm-text>
				</view>
			</view>
 ...
 */
import { computed,watch, PropType, provide,ref,getCurrentInstance, onUnmounted,Ref ,inject, isProxy,toRaw, ComputedRef, onMounted, nextTick } from "vue";
